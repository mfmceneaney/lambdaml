...
clf = GraphClassifier(
    in_dim=64 * (heads if gnn_type=="gat" else 1.0),
    out_dim=num_classes,
    num_layers=3,
    hidden_dim=128,
    dropout=0.4
...
def alpha_fn(epoch, total_epochs):
...
def eval_model(loader,return_labels=False):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            if return_labels:
                logits.extend(F.softmax(out[:,1],dim=0).cpu().tolist())
...
clf_losses, domain_losses, src_accs, lrs = train(epochs=epochs, alpha_fn=alpha_fn)

src_acc, outs, ys = eval_model(src_loader,return_labels=True)
...
alpha_values = [alpha_fn(e, epochs) for e in epoch_range]
...
axs[0, 0].set_title('Discriminator Loss Coefficient',usetex=True)
...
axs[0, 1].plot(epoch_range, domain_losses, label='Domain Loss', color='orange')
...
axs[1, 1].scatter(best_fpr, best_tpr, color='red', marker='*', label=f'Max FOM \n(FOM={best_fom:.2f})\n(Thr={best_thr:.2f})')
...
# Plot model outputs
axs[0, 2].hist(outs, bins=50, alpha=0.6, label="Source Domain", color='skyblue', density=True)
axs[0, 2].hist(tgt_outs, bins=50, alpha=0.6, label="Target Domain", color='salmon', density=True)
