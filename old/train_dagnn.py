...
# Create PyG Dataset
# root = '/work/clas12/users/mfmce/pyg_test_rec_particle_dataset_3_5_25/'
...
#TODO: MERGE DATASETS FOR TRAINING DISCRIMINATOR
...
print("\ndataset_mc[0].pos = ",dataset_mc[0].pos)
...
dropout = 0.0
discriminator = MLP([hidden_dim if idx<nlayers_discriminator-1 else out_channels], norm=None, dropout=dropout, act='relu')

# And then a classifier
nlayers_classifier = 3
dropout = 0.0
classifier = MLP([hidden_dim if idx<nlayers_classifier-1 else out_channels], norm=None, dropout=dropout, act='relu')
...
def get_loaders_dt_mc(dataset):
    ...
    from torch_geometric.loader import DataLoader
    from torch.utils.data import WeightedRandomSampler
    print("DEBUGGING: train_dataset.isdata.shape = ",train_dataset.isdata.shape)
    _, train_counts = np.unique(train_dataset.isdata, return_counts=True)
    print("DEBUGGING: np.unique(train_dataset) = ",_,train_counts)
    train_weights = [1/train_counts[i] for i in train_dataset.isdata]
    train_sampler = WeightedRandomSampler(weights=train_weights, num_samples=len(train_dataset), replacement=True)
    _, val_counts = np.unique(val_dataset.isdata, return_counts=True)
    print("DEBUGGING: np.unique(val_dataset) = ",_,val_counts)
    val_weights = [1/val_counts[i] for i in val_dataset.isdata]
    val_sampler = WeightedRandomSampler(weights=val_weights, num_samples=len(val_dataset), replacement=True)
    _, test_counts = np.unique(test_dataset.isdata, return_counts=True)
    print("DEBUGGING: np.unique(test_dataset) = ",_,test_counts)
    test_weights = [1/test_counts[i] for i in test_dataset.isdata]
    test_sampler = WeightedRandomSampler(weights=test_weights, num_samples=len(test_dataset), replacement=True)
...
loaders_both, datasets_both = get_loaders(dataset_both)
batch_size = 256
use_weighted_samplers = False
...
optimizer_dt = torch.optim.Adam(list(model.parameters(),discriminator.parameters), lr=1e-3)
...
data_labels_dt = train_dataset_dt.isdata
unique_dt, counts_dt = np.unique(data_labels_dt,return_counts=True)
...
weight_dt = torch.FloatTensor([weight_signal_dt, 1.0]).to(device) #NOTE: That labels are [sg?,bg?] so label 0 in this case is sg and label 1 is bg.
...
optimizer_mc = torch.optim.Adam(list(model.parameters(),classifier.parameters), lr=1e-3)
# optimizer = torch.optim.SGD(model.parameters(), lr=0.001, momentum=0.9)
...
data_labels_mc = train_dataset_mc.y
unique_mc, counts_mc = np.unique(data_labels_mc,return_counts=True)
...
# Define training and testing routines
def train_discr(model,optimizer,loader,criterion):
    model.train()
...
# Define training and testing routines
def train_da(model,classifier,optimizer,train_loader,criterion,discriminator,optimizer_discr,criterion_discr,alpha):
    model.train()
    classifier.train()
...
# #         edge_index = data.edge_index
# #         batch = data.batch
        out_model = model(data.x, data.edge_index, data.batch) # Perform a single forward pass.
...
for epoch in range(nepochs):
    print("BEFORE TRAIN()")
    train_discr(model_dt,optimizer_dt,train_loader_dt,criterion_dt)
    print("BEFORE TEST(TRAIN_LOADER)")
    train_acc, train_loss, train_precision, train_recall, train_precision_n, train_recall_n, _, _, __, train_roc_auc = test_discr(model_dt,optimizer_dt,train_loader_dt,criterion_dt)
    ...
    val_acc, val_loss, val_precision, val_recall, val_precision_n, val_recall_n, _, _, __, val_roc_auc = test_discr(model_dt,optimizer_dt,val_loader_dt,criterion_dt)
...
#             }, PATH)
        ...
        torch.save({
            ...
            'optimizer_state_dict': optimizer_discr.state_dict(),
#             'loss': loss,
            }, PATH)
...
plt.xlabel('Epoch')
plt.ylabel('Recall')
f.savefig('recall_discr.pdf')
f.close()

# Save logs to yaml
import yaml

with open('train_metrics_discr.yaml', 'w') as file:
    yaml.dump(train_metrics, file)
with open('val_metrics_discr.yaml', 'w') as file:
...
for epoch in range(nepochs):
    print("BEFORE TRAIN()")
    train_da(model,classifier,optimizer_mc,train_loader_mc,criterion_mc,discriminator,optimizer_dt,criterion_dt,alpha)
    print("BEFORE TEST(TRAIN_LOADER)")
    train_acc, train_loss, train_precision, train_recall, train_precision_n, train_recall_n, _, _, __, train_roc_auc = test(model_mc,optimizer_mc,train_loader_mc,criterion_mc)
    train_metrics["acc"].append(train_acc)
    train_metrics["loss"].append(train_loss)
    train_metrics["precision"].append(train_precision)
    train_metrics["recall"].append(train_recall)
    train_metrics["roc_auc"].append(train_roc_auc)
    print("BEFORE TEST(VAL_LOADER)")
    val_acc, val_loss, val_precision, val_recall, val_precision_n, val_recall_n, _, _, __, val_roc_auc = test(model_mc,optimizer_mc,val_loader_mc,criterion_mc)
    if epoch==0 or val_roc_auc>np.max(val_metrics["roc_auc"]):
...
with open('train_metrics.yaml', 'w') as file:
    yaml.dump(train_metrics, file)
with open('val_metrics.yaml', 'w') as file:
    yaml.dump(val_metrics, file)

with open('train_metrics_da.yaml', 'w') as file:
    yaml.dump(train_metrics_da, file)
with open('val_metrics_da.yaml', 'w') as file:
    yaml.dump(val_metrics_da, file)


#---------- NOW RUN ON TEST DATASETS ----------#

train_acc, train_loss, train_precision, train_recall, train_precision_n, train_recall_n, outputs, decisions, y_true, roc_auc_test = test(model_mc,optimizer_mc,test_loader_mc,criterion_mc)
import sklearn
...
#     plt.hist(array_bg_true, color='tab:blue', alpha=0.5, range=(low,high), bins=nbins, histtype='stepfilled', density=False, label='bg true')
    ...
    f.savefig(xlabel+'__mctest__sg.pdf')
#     f.savefig(xlabel+'_separated_'+todays_date+'.pdf')
    ...
    plt.xlabel(xlabel)
    if logy: plt.yscale('log')
    f.savefig(xlabel+'__mctest__sg_bg.pdf')
#     f.savefig(xlabel+'_separated_'+todays_date+'.pdf')
...
train_loss, outputs, decisions = data_test(model_mc,optimizer_mc,test_loader_dt,criterion_mc)
...
#     plt.hist(array_bg, color='tab:blue', alpha=0.5, range=(low,high), bins=nbins, histtype='stepfilled', density=False, label='bg')
    ...
    plt.xlabel(xlabel)
    if logy: plt.yscale('log')
    f.savefig(xlabel+'__dttest__sg.pdf')
#     f.savefig(xlabel+'_data_separated_'+todays_date+'.pdf')
    ...
    plt.xlabel(xlabel)
    if logy: plt.yscale('log')
    f.savefig(xlabel+'__dttest__sg_bg.pdf')
#     f.savefig(xlabel+'_data_separated_'+todays_date+'.pdf')
