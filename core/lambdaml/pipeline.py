...
def pipeline_da(
    ...
    disc_path="projector.pt",
    disc_params_path="projector_params.json",
    # Optuna trial
    trial=None,
    ...
    # Save discriminator
    logger.info("Saving Discriminator")
    torch.save(disc.state_dict(), osp.join(out_dir, disc_path))

    # Save discriminator parameters to json
    logger.info("Saving Discriminator Parameters")
    save_json(
        osp.join(out_dir, disc_params_path),
        {
            "in_dim": hdim_gnn * (heads if gnn_type == "gat" else 1),
            "num_layers": num_layers_disc,
            "hdim": hdim_disc,
    ...
    loss_coeffs = [alpha_values]
    loss_coeffs_kwargs = [{"label": "alpha"}]
...
def pipeline_can(
    ...
    projector_params_path="projector_params.json",
    # Optuna trial
    trial=None,
    metric_fn=lambda logs: logs[0]["auc"],  # Available logs are [val_logs]
):
    ...
    projector = ProjectionHead(
        hdim_gnn * (heads if gnn_type == "gat" else 1), projector_heads
    ).to(device)
    ...
    # Save projector
    logger.info("Saving Projector")
    torch.save(projector.state_dict(), osp.join(out_dir, projector_path))

    # Save projector parameters to json
    logger.info("Saving Projector Parameters")
    save_json(
        osp.join(out_dir, projector_params_path),
        {
            "in_dim": hdim_gnn * (heads if gnn_type == "gat" else 1),
            "out_dim": projector_heads,
        },
    )
