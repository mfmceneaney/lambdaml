...
class LazyDataset(Dataset):
    ...
    def __init__(self, root, transform=None, pre_transform=None, pre_filter=None, datalist=None, num_workers=0, chunk_size=100, pickle_protocol=5, clean_keys=('is_data', 'rec_indices'), batch_size=100000, drop_last=False, weights_only=False, recreate=False):
        ...
        metadata = {"size":self.size, "batch_size": self.batch_size, "num_batches":self.num_batches}
        if not recreate and osp.exists(metadata_path):
            ...
            try:
                with open(metadata_path, "r") as f:
                    ...
                    self.num_batches = self.size//self.batch_size+(1 if self.size%self.batch_size>0 and not self.drop_last else 0)
            ...
            _loaded_batch = torch.load(os.path.join(self.processed_dir, self.processed_file_names[self.process_batch_start_idx]), weights_only=self.weights_only)
            if len(_loaded_batch)!=self.batch_size:
                if self.datalist is None: self.datalist = []
    ...
    def save_graph_batch(self,idx):
        ...
        torch.save(data, osp.join(self.processed_dir, self.processed_file_names[idx+self.process_batch_start_idx]), pickle_protocol=self.pickle_protocol)
    ...
    def process(self):
        ...
        if self.num_workers<=0:
            ...
            with multiprocessing.Pool(processes=min(local_num_batches, self.num_workers)) as pool:
                try:
                    list(tqdm(pool.imap_unordered(self.save_graph_batch, range(local_num_batches), self.chunk_size), total=local_num_batches))
                except KeyboardInterrupt as e:
                    print("Caught KeyBoardInterrupt, terminating workers")
                    pool.terminate()
                    pool.join()
...
def label_rec_particle(data_event_tables,**kwargs):
    ...
    if not rec_particle_bank_name in data_event_tables and mc_lund_bank_name in data_event_tables:
        return torch.tensor([0], dtype=torch.long)

    # Get REC::Particle bank
    rec_particle_event_table = data_event_tables[rec_particle_bank_name] #NOTE: This should always be added
...
def get_kinematics_rec_particle(data_event_tables,**kwargs):
    ...
    rec_kinematics_event_table = data_event_tables[rec_kinematics_bank_name] if add_kinematics else None
            
    # Select first entry for REC::Kinematics #TODO: Think about this.
    rec_kinematics_event_table = np.take(rec_kinematics_event_table,[0],axis=0) #NOTE: ONLY LOOK AT FIRST ENTRY FOR COMPATIBILITY
...
def pipeline_preprocessing(
    ...
    for batch_num, batch in tqdm.tqdm(enumerate(hp.iterate(file_list,banks=banks,step=step))):
        ...
        for event_num, _ in enumerate(range(0,len(batch[list(batch.keys())[0]]))):
            ...
            # Get data tables
            data_event_tables = {bank: get_event_table(bank_keys[bank],event_num,batch,dtype=float) for bank in banks}
            ...
            if callable(preprocessing_fn):
                ...
                raise TypeError("Preprocessing function must be calllable, but found type(preprocessing_fn) =",type(preprocessing_fn))
            ...
            if kinematics_fn is not None and callable(kinematics_fn):
                data.kinematics = kinematics_fn(data_event_tables,**kinematics_fn_kwargs)
...
#TODO: ADD argparse SCRIPTS for dataset creation and hyperparameter optimization maybe a job setup script too for these
#TODO: FIGURE OUT HOW TO LOG TO WANDB EVEN ON FARM
